...
@section('Titulo', 'Sage2.0 - Titulos')
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="card card-lightblue">
                        ...
                        <!-- /.card-header -->
                        <form method="POST" action="{{ route('formularioTitulos') }}" class="formularioTitulos">
                            @csrf
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="Descripcion">Descripción del Titulo</label>
                                    <input type="text" class="form-control" id="Descripciontitulo" name="DescripcionTitulo" placeholder="Ingrese Descripción del Titulo" value="">
                                </div>
                            </div>
                        ...
                        <div class="card-body table-responsive">
                                    ...
                                    <tr class="gradeX">
                                        ...
                                        <td>
                                            <div class="row" style="display: flex;justify-content:center;gap: 10px">
                                                <a class="d-flex justify-content-center edit-btn" href="#" title="Editar" data-id="{{$o->idTitulo}}" data-nombre="{{$o->nombre_titulo}}" data-estado="{{$o->estado_titulo}}" data-toggle="modal" data-target="#editModal">
                                                ...
                                                @php
                                                    ...
                                                    $permiteBorrar = $permiteBorrar = DB::connection('DB2')
                                                                    ->table('tb_titulos')
                                                                    ->join('tb_registro_de_titulos', 'tb_registro_de_titulos.nombre_titulo', '=', 'tb_titulos.nombre_titulo')
                                                                    ->where('idTitulo', $o->idTitulo)
                                                                    ->count();

                                                @endphp
                                                @if ($permiteBorrar == 0)
                                                    <a class="eliminar-enlace d-flex justify-content-center" href="#" title="Eliminar" data-id="{{$o->idTitulo}}">
                            ...
                            <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"     aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form id="editForm" action="{{route('formularioActualizarTitulos')}}" method="POST">
                                        @csrf
                                        <div class="modal-body">
                                                <div class="form-group">
                                                    <label for="tituloId">ID</label>
                                                    <input type="text" class="form-control" id="tituloId" readonly>
                                                    <input type="hidden" id="tituloidEnv" name="tituloidEnv" value="">
                                                </div>
                                                <div class="form-group">
                                                    <label for="tituloNombre">Nombre del Título</label>
                                                    <input type="text" class="form-control" id="tituloNombre" name="Descripcion">
                                                </div>
                                                <div class="form-group">
...
<script src="{{ asset('js/funcionesvarias.js') }}"></script>
        @if (session('ConfirmarNuevoTitulo')=='OK')
            <script>
            Swal.fire(
                'Registro guardado',
                'Se cargo correctamente un nuevo titulo',
                'success'
                    )
            </script>
        @endif
        @if (session('ConfirmarActualizarTitulo')=='OK')
        <script>
        Swal.fire(
            'Registro Actualizado',
            'Se actualizo correctamente un titulo',
            'success'
                )
        </script>
    @endif
    ...
    $('.formularioTitulos').submit(function(e){
        e.preventDefault();
        
        if(document.getElementById('Descripciontitulo').value === ""){
            Swal.fire(
                'Alerta',
 ...
 <script src="{{ asset('js/funcionesvarias.js') }}"></script>
        @if (session('ConfirmarBorradoTitulo')=='OK')
            <script>
            Swal.fire(
        ...
        @endif
        @if (session('ConfirmarBorradoTituloFallido')=='OK')
        <script>
        Swal.fire(
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                       // Si el usuario confirma, redirige a la URL deseada
                        var url = "{{ route('eliminarTitulo', ':id') }}";
