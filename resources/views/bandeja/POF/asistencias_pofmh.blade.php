...
<section id="container" >
    <section id="main-content">
        <section class="content-wrapper">
            <h2 id="tituloAsistencia">Planilla de Asistencias</h2>
            ...
            <script>
                ...
                <table id="POFMHAsistencia">
                    @php
                        ...
                        $mesActual = Carbon::now()->month; // Obtener el mes actual
                        $anioActual = Carbon::now()->year; // Obtener el año actual

                        $mesActualArray = [];

                        // Obtener el número de días del mes actual
                        $numeroDias = Carbon::create($anioActual, $mesActual)->daysInMonth;

                        for ($i = 1; $i <= $numeroDias; $i++) {
                            // Crear una fecha para el día correspondiente del mes actual
                            $fecha = Carbon::create($anioActual, $mesActual, $i);
                            
                            // Obtener el día de la semana en inglés (Mon, Tue, etc.)
                            $diaIngles = $fecha->format('D');
                            
                            // Convertir el día de la semana a su abreviatura en español
                            $abreviatura = $diasSemana[$diaIngles];
                            
                            // Asignar la abreviatura al arreglo
                            $mesActualArray[$i] = $abreviatura;
                        }
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Division</th>
                            <th>Horas</th>
                            @foreach ($mesActualArray as $dia => $abreviatura)
                                @php
                                    // Formatear la fecha del día actual del ciclo a 'Y-m-d'
                                    $fechaActual = date('Y-m') . '-' . str_pad($dia, 2, '0', STR_PAD_LEFT);
                                @endphp
                        ...
                        @foreach ($infoPofMH as $pof)
                            ...
                            @endphp
                                <tr>
                                    ...
                                    @foreach ($mesActualArray as $dia => $abreviatura)
                                        @php
                                            $asistencia = $Asistencias->filter(function($item) use ($dia, $pof) {
                                                return $item->dia == $dia 
                                            ...
                                            $currentDate = date('Y-m') . '-' . str_pad($dia, 2, '0', STR_PAD_LEFT);
                                        @endphp
                                        ...
                                        @else
                                            @if ($abreviatura == 'D')
                                                <td class="attendance-cell " data-day="{{ $dia }}" data-date="{{ $currentDate }}" id="finde" onclick="openModal(this)">
                                                    -
                                                </td>
                                            @else
                                                ...
                                                @else
                                                    <td class="attendance-cell {{ $cellClass }}" data-day="{{ $dia }}" data-date="{{ $currentDate }}" id="laboral" onclick="openModal(this)">
                                                        {{ $tipo ? $tipo : '' }}
                                                    </td>
                                                @endif
                                            @endif
...
<script>
    ...
    var noviembre = @json($mesActualArray);
</script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (status === 'C') {
                    ...
                    selectedCell.className = 'attendance-cell';
                    const selectedRow = selectedCell.parentElement;
                    applyLicenseHighlightNueva(selectedRow, FechaInicio, FechaHasta,'','#f1f2f7');
                }
                if (status === 'L') {
                    const selectedRow = selectedCell.parentElement;
...
$(document).ready(function () {
    ...
    function searchPofmhCompleto(query, mostrarTodo) {
        $.ajax({
            url: '/buscarPofmhCompleto/' + idExtension,
            type: 'GET',
    ...
    function fillTable(data) {
        ...
        $.each(data.pofmh, function (index, item) {
            if (item.Agente !== null) {
                ...
                const mesActualNombre = nombresMeses[today.getMonth()];

                // Obtener el número de días del mes actual
                const numeroDias = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate(); // Obtener número de días del mes actual

                // Crear el array para el mes actual
                let mesActualArray = [];
                ...
                for (let dia = 1; dia <= numeroDias; dia++) {
                    ...
                    const abreviatura = mesActualArray[dia]; // Asegúrate de definir `noviembre` antes de usarlo
                    ...
                    if (diasBloqueados.has(fechaFormateada)) {
                        ...
                        if(tipo === 'A') {
                            rowHtml += `<td class="attendance-cell ${cellClass}" data-day="${dia}"  onclick="openModal(this)" id="laboral"></td>`;
                        } else {
                            rowHtml += `<td class="attendance-cell ${cellClass}" data-day="${dia}" id="laboral" onclick="openModal(this)">${tipo}</td>`;
                        }
                    }
                }
    ...
    // Mostrar el título con el mes y la fecha actual
    document.getElementById("tituloAsistencia").innerText = `Planilla de Asistencia - Mes: ${mesActual} - Fecha Actual: ${fechaFormateada}`;
