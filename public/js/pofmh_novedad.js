...
if (previewNode) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        $("#modal-novedades").on("show.bs.modal", function (event) {
            var button = $(event.relatedTarget); // Obtén el botón que disparó el evento
            var agente = button.data("agente"); // Obtiene el valor de data-agente
            var cue = document.getElementById("valCUE").value; // Obtiene el valor de CUE desde el input oculto
            // Almacenar los valores para usarlos después
            window.agente = agente; // Guarda el agente en una variable global
            window.cue = cue; // Guarda el CUE en una variable global

            console.log("Agente capturado:", agente);
            console.log("CUE capturado:", cue);
            // Agregar los parámetros antes de enviar el archivo

            //lleno la lista
            traerArchivos2();
            myDropzone.on("sending", function (file, xhr, formData) {
                formData.append("_token", csrfToken); // Asegúrate de tener el token CSRF
                formData.append("Agente", agente); // Enviar Agente (DNI) desde data-agente
                formData.append("CueX", cue); // Enviar CueX desde el input oculto con el CUE

                // Para depuración: verificar los valores que se están enviando
                console.log("Enviando archivo con Agente:", agente);
                console.log("Enviando archivo con CueX:", cue);
            });
        });
...
document.addEventListener("DOMContentLoaded", function () {
    // Selecciona todas las celdas de la tabla con la clase attendance-cell
    const cells = document.querySelectorAll(
        "#POFMHAsistencia .attendance-cell"
    );

    cells.forEach((cell) => {
        // Agrega el evento para cuando el mouse entre en la celda
        cell.addEventListener("mouseenter", function () {
            cell.classList.add("highlighted-cell");
        });

        // Agrega el evento para cuando el mouse salga de la celda
